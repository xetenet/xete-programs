...
**Upgrade authority:** `<REDACTED-WALLET>` — keypair at `<upgrade-authority-keypair.json>` (gitignored)
...
- [x] Mainnet live-state audit: only ONE account under the program — the Config PDA `<REDACTED-WALLET>` (108 bytes). No live swaps/offers/listings.
- [x] Migration safety: live Config parses correctly under the new layout (admin=<REDACTED-WALLET>, fee_wallet=xtFEE, fee_bps=30, delist=0.001 SOL, shards=0, alias-gate OFF).
...
- Deploy wallet <REDACTED-WALLET> balance at audit: **0.613 SOL → INSUFFICIENT.** Top up to **~3 SOL** before deploying.

## Deploy steps (PAID)
1. Confirm wallet funded (≥ ~3 SOL): `solana balance <REDACTED-WALLET>… --url mainnet-beta`
...
4. **Upgrade (auto-extends, retains authority — do NOT pass `--final`):**
   ...
   solana program deploy --program-id AXTSWAPVuUPivum39Fh2N6AS6SsktpABDpBLePknyvUD \
     ...
     --url mainnet-beta
   ```
5. Verify: `solana program show AXTSWAPVuUP… --url mainnet-beta` → Data Length 315688, Authority <REDACTED-WALLET>, new Last Deployed slot.
...
Instant, read-only deploy verification (zero SOL):
  ...
  -> PASS when Data Length=315688, on-chain hash=f25e9461…, authority=<REDACTED-WALLET> retained, config(108B) survives.
